...
label4Nicole <- function(orig.label , image.f = RawFiles[f]){
  ...
  MakeLabelMarker <- function(){
    ...
    for( i in 0:199 ){
      ...
      #plot(0:1,0:1,type="n")
      text(0.5,0.5,i,col='#FF0000' , cex= 6)
    }
    dev.off()
    #par(par.default) 
  ...
  print(paste("DEBUG" , Sys.time() , ": output image " , paste0(image.f,"_labeled.jpg") ) )
  writeImage(RawImage, paste0(image.f,"_labeled.jpg"))
}
...
if(length(RawFiles) == length(ResultsFiles)){
  for(f in c(1,2,4:length(RawFiles))) {
    print(Sys.time())
    print(RawFiles[f])
...
try(label4Nicole(labels,gsub(".JPG","_LeafLabel.jpg",RawFiles[f])))
	}
}
